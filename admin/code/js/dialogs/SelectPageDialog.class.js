...
var SelectPageDialog = {
    ...
    paint : function(){
		...
		txt += "    Make sure you set the A-RECORD to ApolloSites IP address (<b>"+SelectPageDialog.apolloIP+"</b>).";
		...
		txt += "    <button class='basic_button' onclick='SelectPageDialog.changeDomain()'>Change</button>"
		txt += "    </div>";
		
		txt += "    <div class='dataWrapper'>";
		txt += "    <span class='dataLabel'>Email</span><input id='userEmail' class='dataValue' type='text' />";
		txt += "    <button class='basic_button' onclick='SelectPageDialog.changeEmail()'>Change</button>"
		txt += "    </div>";
		...
		txt += "    <button class='basic_button' onclick='SelectPageDialog.changePassword()'>Change</button>"
		txt += "    </div>";
		...
		for (var m=1; m<=12; m++){
		txt += "            <option value='"+m+"'>"+m+" - " + SelectPageDialog.monthToName(m) + "</option>";
		}
		txt += "        </select>";
		...
		txt += "    <button class='basic_button' onclick='SelectPageDialog.changeCard()'>Update</button>"
		txt += "</div>";
		...
		$('#cardNumber').typing({ stop: SelectPageDialog.onCardNumber, delay: 400});
	...
	changePassword : function(){
		...
		BillingAPI.changePassword(ssMain.siteID, oldPassword, newPassword, SelectPageDialog.onPasswordChanged);
	...
	changeEmail : function(){
		...
		if (SelectPageDialog.userEmail == newEmail){
	...
	changeDomain : function(){
		...
		var ip = SelectPageDialog.apolloIP;
		var newDomain = $('#userDomain').val();
		
		if (SelectPageDialog.userDomain == newDomain){
		...
		AthenaDialog.confirm(msg, SelectPageDialog.doChangeDomain);
	},
	...
	onCardNumber : function(){
		...
		var type = SelectPageDialog.getCardTypeByNumber(number);
	...
	isValidCC : function(str) { //A boolean version
    	if (SelectPageDialog.getCardTypeByNumber(str) == '?') return false;
    	return true;
	}    
}