...
class ThemeTable {
	...
	*/
	public static function getNumberGalleriesForMultiGallery($page_post_id){
		...
		$sql = DatabaseManager::prepare("SELECT para_value FROM apollo_PageParas gp INNER JOIN apollo_ThemeParas tp WHERE page_post_id = %d AND gp.theme_para_id = tp.id AND tp.para_type = 'multi-gallery'", $page_post_id);
